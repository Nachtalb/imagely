$(document).ready(function () {
    ...
    $('form input[name=\'PasswordRepeat\']').on('change', function () {
        var inputName = $(this).attr('name'); //stores name of inputfield
        var inputValue = $(this).val(); //stores value of inputfield
        removeStatus(inputName + 'Empty');
        statusPasswordRepeatEmpty = false;
        if (inputValue !== $('form input[name=\'Password\']').val()) {
            $(this).closest('div.form-group').addClass('has-error');
            if (statusPasswordRepeatValidation === false) {
                addStatus('danger', inputName + 'Validation', 'Bitte wiederholen Sie das Passwort korrekt');
                statusPasswordRepeatValidation = true;
            }
        } else {
            $(this).closest('div.form-group').removeClass('has-error');
            if (statusPasswordRepeatValidation === true) {
                removeStatus(inputName + 'Validation');
